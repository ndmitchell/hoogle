...
download x = do
    ...
    when (not b) $ do
        if x == "base" then do
         ...
         else if x == "keywords" then do
            createDirectoryIfMissing True "keywords"
            system_ "wget http://haskell.org/haskellwiki/Keywords -O temp/keywords/keywords.html"
         else do
            ...
            system_ $ "wget " ++ link ++ " -O temp/" ++ x ++ ".tar.gz"
